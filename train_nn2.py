...
class Net2(nn.Module):
    ...
    def __init__(self, input_num):
        ...
        self.fc4 = nn.Linear(HIDDEN_NUM, 2)
...
if __name__ == "__main__":
    ...
    for epoch in range(20):
        for step, (batch_x, batch_y) in enumerate(train_loader):
            batch_x = batch_x
            # batch_y = torch.cat((batch_y, 1-batch_y), -1)
            batch_y = 1 - batch_y
            out = net(batch_x)
    ...
    ypred = net(test_x)[:, :1].squeeze().detach().numpy()
    y_pred = (ypred >= 0.5)*1
