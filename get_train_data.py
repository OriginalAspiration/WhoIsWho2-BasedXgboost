...
def f(negative_example, existing_data_hash_by_name, train_pub, kdd_data, kdd_data_triplet, k2k_edges, n2n_edges, pool_id, is_negative):
...
if __name__ == "__main__":
    ...
    for one_data in positive_example:
        ...
        if len(sub_data) >= step:
            jobs.append(pool.apply_async(f, args=(sub_data, existing_data_hash_by_name, 
                                train_pub, kdd_data, kdd_data_triplet, k2k_edges, n2n_edges, id, False)))
            id += 1
            sub_data = []
    ...
    for one_data in negative_example:
        ...
        if len(sub_data) >= step:
            jobs.append(pool.apply_async(f, args=(sub_data, existing_data_hash_by_name, 
                                        train_pub, kdd_data, kdd_data_triplet, k2k_edges, n2n_edges, id, True)))
            id += 1
            sub_data = []
